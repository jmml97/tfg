...
% TODO: Ideales a izquierda y derecha
...
Los cuerpos finitos también se suelen conocer como \textit{cuerpos de Galois}, en honor a Évariste Galois, uno de los primeros matemáticos en trabajar con ellos. 
Los notaremos por \(\mathbb F_p\), donde \(p\) es el orden del cuerpo.
...
% TODO: añadir que todos los cuerpos finitos tienen cardinal p^r, donde p es la característica
...
En esta subsección vamos a construir cuerpos finitos a partir del cociente de anillos de polinomios.
Cuando realizamos el cociente por un tipo de polinomios concreto, llamados irreducibles, vamos a comprobar que el anillo cociente que obtenemos tiene estructura de cuerpo.
...
%\end{proof}
Tomamos por tanto un polinomio \(f(x) \in \mathbb F_p[x]\) que sea irreducible sobre \(\mathbb F_p\) y de grado \(m\).
Consideramos el anillo cociente dado por \(\mathbb F_p[x]/(f(x))\).
Utilizando el algoritmo de Euclides podemos comprobar que es un cuerpo de característica \(p\), y de hecho, es un cuerpo finito con \(q = p^m\) elementos.
Al tratarse de un cociente todo elemento es una clase lateral y será de la forma \(g(x) + (f(x))\), donde el polinomio \(g(x)\) es único y con grado como mucho \(m-1\).
...
Por el teorema \ref{th:el-Fq-raices-pol} cada elemento de \(\mathbb F_{q^t}\) es raíz del polinomio \(\cramped{x^{q^t}} - x\).
