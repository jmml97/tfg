...
% Ideales biláteros, teorema del centro

% right divisor, left multiple
Para ser más concisos con la notación vamos a llamar \(R = F_q[x, \sigma]\) y cuando no sea necesario hacer referencia a la variable \(x\), a un polinomio \(f(x)\) lo denotaremos simplemente por \(f\).
...
Definimos la \emph{norma} \(i\)\emph{-ésima} de un elemento \(b \in \mathbb F_q\) como
\[
  N_i(b) = (\sigma^{i-1}b)\dots (\sigma b)b \quad\text{para } i > 0 \quad\text{y } N_0(b) = 1.
\]

\begin{proposition}
  Si \(f(x) = \sum_0^n a_ix^{n-i} \in \mathbb F_q[x, \sigma]\) y \(b \in \mathbb F_q\) entonces \((x - b) \mid_d f(x)\) si y solo si \(\sum_0^n a_iN_{n-i}(b) = 0\).
\end{proposition}
