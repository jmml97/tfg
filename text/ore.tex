...
\begin{proof}
  ...
  \begin{align*}
    x^{i} &- \sigma^{i-1}(\gamma)\sigma^{i-2}(\gamma) \cdots \gamma\\
      &= (x^{i-1} + \sigma^{i-1}(\gamma)x^{i-2} + \dots + \sigma^{i-1}(\gamma)\cdots\sigma(\gamma))(x - \gamma)
  \end{align*}
  para \(i = 1, 2, \dots\). 
  Si multiplicamos por la izquierda por \(a_{n-i}\) y sumamos en \(i\) obtenemos que
  \[
    f(x) = q(x)(x - \gamma) + a_0N_n(\gamma) + a_1N_{n-1}(\gamma) + \dots + a_nN_0(\gamma),
  \]
  por lo que el resto de dividir \(f(x)\) por \((x - \gamma)\) por la derecha es 
  \[
    a_0N_n(\gamma) + a_1N_{n-1}(\gamma) + \dots + a_nN_0(\gamma),
  \]
  de lo que deducimos lo que queremos demostrar.
\end{proof}
