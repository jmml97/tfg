...
Por el lema \ref{lem:pgz-sesgados-rangos} es el mayor entero \(r \leq t\) tal que \(E^r\) y \(\Sigma E^r\) tienen rango máximo.
Denotaremos por \(\mu\) a tal máximo.

\begin{lemma}
  \label{lem:pgz-sesgados-rango-mu}
  Para cada \(\mu \leq r \leq t\) el máximo \(\mu = \rank(E^r) = \rank(S^r)\).
  Además \(\mu \leq v\).
\end{lemma}
