...
\begin{definition}
  Bajo las condiciones y notación de este capítulo, un \emph{código \textacr{RS} (Reed-Solomon) sesgado} de distancia mínima prevista \(\delta\) es un código cíclico \(\mathcal C\) tal que \(\mathfrak v^{-1}(\mathcal C)\) está generado por 
  \[
...
\begin{theorem}
  ...
  Un código \textacr{RS} sesgado de distancia mínima prevista \(\delta\) tiene distancia mínima \(\delta\).
\end{theorem}
