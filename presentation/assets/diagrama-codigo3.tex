...
\begin{tikzpicture}[
  ...
  \node (c2) [right=2.5cm of z, nobordertypetag] { 111 000 };
  \node (b2) [above=of c2.west, typetag] { 000 111 };
  \node (a2) [draw, above=of b2.west, nobordertypetag] { 000 000 };
  \node (d2) [below=of c2.west, nobordertypetag] { 111 111 };
  %\node (e2) [below=of d2.west, nobordertypetag] { E };

  \node (codigo2) [above=of a2, title] { Código };
  
  \node [draw=black!50, ellipse, minimum width=2cm, fit={(codigo2) (a2) (b2) (c2) (d2)}] {};

  \node (mensaje2) [right= 1.5cm of c2, title, anchor=west] { Mensaje };
  \node (m2) [below=of mensaje2.west, nobordertypetag] {01};

  \node (mensajecaja2) [draw=black!50, rectangle, fit={(mensaje2) (m2)}] {};

  \draw [<->, very thick] (mensaje) -- (b);
  \draw [dotted] (b) -- node[below, font=\scriptsize] { transmisión } (z);
  \draw [->] (z) -- node[below, font=\scriptsize] { corrección } (b2);
  \draw [<->] (b2) -- (mensaje2);
\end{tikzpicture}
